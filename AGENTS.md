...
### 🔍 Teljes SEO/GEO Audit & vercel.app Duplikált Domain Javítás (2026.08.25)
- **Cél:** Az ügyfél kérésére teljes körű audit a Google keresési láthatóság érdekében – kód, Google Search Console, Google Cégprofil, Facebook oldal, éles domain.
- **Kód-oldali állapot:** a korábbi SEO-hullámok (2026-08-13/15/22) miatt a metaadatok, JSON-LD sémák (`AutoDealer`, `WebSite`, `Car`, `ItemList`, `Service`, `BreadcrumbList`), a `sitemap.ts` és a `robots.ts` már jó állapotban voltak – ez az audit ezt megerősítette, érdemi kódhibát csak egy helyen talált (lásd lent).
- **Google Search Console találatok:**
  - Sitemap rendben (17 URL felfedezve, sikeres beolvasás).
  - Indexelés: 10 oldal indexelve, 17 nem indexelt – ebből 5 a szándékos 308 átirányítás (legacy URL-ek), 1 a friss `bmw-x6-xdrive30d-2012` (feltérképezve, indexelésre vár), 11 pedig **"Felfedezve – jelenleg nincs indexelve"** állapotban ragadt: `/kapcsolat`, `/autobeszamitas`, és a jármű-adatlapok nagy része (`seat-alhambra-2015`, `citroen-jumpy-spacetourer-2017`, `jaguar-f-pace-2017`, `skoda-octavia-2022-kezi` stb.) – ez fiatal domainnél (a property 2026-08-12 óta létezik) várható jelenség, a Google még nem térképezte fel teljesen a kínálatot.
  - **Manuális újraindexelés kérve** az URL-ellenőrző eszközzel: `/kinalat/seat-alhambra-2015`, `/kinalat/citroen-jumpy-spacetourer-2017`, `/kinalat/jaguar-f-pace-2017`, `/kinalat/skoda-octavia-2022-kezi`, `/autobeszamitas`. A `/kapcsolat`-ra és a `/kerekcsere`-re a 2026-08-22-i munkamenetben már történt kérés, ezért ezúttal kihagyva (a napi kvóta védelme és a felesleges duplikált kérés elkerülése miatt).
  - Teljesítmény (utolsó 30 nap): 78 kattintás, ~1240 megjelenés, átlagos pozíció 5,1. Vezető lekérdezések: *"autókereskedés zalaegerszeg"* (19 kattintás, 160 megjelenés) és *"zalaegerszeg autókereskedés"* (16, 193). A *"kerékcsere/gumicsere/gumiszerviz zalaegerszeg"* és a márkaspecifikus *"használt BMW eladó zalaegerszeg"* típusú kulcsszavakra még nincs mérhető megjelenés – ez összhangban van azzal, hogy ezek az oldalak/adatlapok még nincsenek teljesen indexelve (lásd fent), várhatóan az indexelés előrehaladtával javul.
  - Strukturált adatok (Bővítések/Vásárlás riportok): **0 hiba** – Termékkivonatok 4/4, Kereskedői adatok 4/4, Navigációs útvonalak 5/5, Véleményrészletek 2/2 mind érvényes.
- **🔴 Kritikus technikai SEO hiba – javítva:** a projekt alapértelmezett `cars-sr99.vercel.app` Vercel-domainje **önállóan indexelve volt a Google-ben**, a saját domainnel párhuzamos, részben elavult tartalommal (pl. a régi, lapos `/skoda-octavia-2022-kezi` URL-szerkezettel) – ez duplikált tartalomként versengett a kanonikus `www.cars-sr99.com`-mal, megosztva a rangsorolási jelzéseket. Javítás: `next.config.ts` `redirects()`-ben egy `has: [{ type: "host", value: "cars-sr99.vercel.app" }]` feltételes szabály minden vercel.app-os kérést 308-cal a kanonikus domainre irányít. **Fontos:** Next.js 16-ban a `middleware.ts` át lett nevezve `proxy.ts`-re – erre a javításra végül nem is volt szükség, mert a `next.config.ts` `redirects()` `has: { type: "host" }` feltétele natívan támogatja a domain-alapú átirányítást, proxy/middleware nélkül.
- **Google Cégprofil:** "Profilerősség: Jónak tűnik", 78 ügyfél-interakció, 9 olvasatlan vélemény (az ügyfélnek érdemes elolvasnia és válaszolnia). A cégprofil-szerkesztő panel böngészőn belüli automatizálása nem volt megbízható (a modal görgetése kiszámíthatatlan volt), ezért a részletes mezőnkénti (kategória/attribútum) ellenőrzés a nyilvános keresési panelre korlátozódott – ott minden alapadat (cím, telefon, "Nyitva" állapot) helyesnek mutatkozott.
- **Facebook oldal (facebook.com/CARS SR99 Kft):** kategória (Autókereskedés), cím (Ságodi Iparterület), telefon (+36 70 907 0669) és weboldal-link mind konzisztens a webhellyel – jó NAP-egyezés. **Egy inkonzisztencia maradt nyitva:** a nyitvatartás *"Mindig nyitva"*-ra van állítva a valós H-P 9:00–17:00 helyett. A javítást böngészőben megkíséreltük, de a Facebook oldalszerkesztő modal-ja az automatizálás alatt megbízhatatlanul viselkedett (véletlenszerűen bezáródott, a görgetés kiszámíthatatlan volt) – **ez manuális javítást igényel**: Oldal kezelése → Névjegy → Részletek → Nyitvatartás → Módosítás → "Meghatározott időszakokban van nyitva" → H–P 09:00–17:00, Szo–V zárva.
- **Cégjegyzék-adatok (Céginfo.hu, OPTEN, Céginformáció.hu):** a hivatalos székhely *"8900 Zalaegerszeg, Vörösmarty Mihály utca 4"*, ami eltér a weboldalon/Cégprofilban szereplő fizikai telephelytől (*"Ságod hrsz. 807/15"*). Ez **nem hiba** – Magyarországon a bejegyzett székhely és a tényleges üzleti telephely (ahol az ügyfelek megjelenhetnek) gyakran eltér, és a Google Cégprofilnak pontosan a fizikai telephelyet *kell* mutatnia, ahogy jelenleg is teszi.
- A teljes megállapítás-lista és a marketing akcióterv (kerékcsere/gumicsere kampány + autóértékesítési kampány) az ügyfélnek átadott, megosztható artifactban készült el ugyanebben a munkamenetben.
- **Még ugyanezen a napon, az audit két nyitott javaslatának megvalósítása:** új `components/FeaturedPicksToday.tsx` – a főoldalon, a hero alatt, naptári nap szerint determinisztikusan rotáló 3 autó ("Ma kiemelten ajánljuk"), hogy minden jármű-adatlap rendszeresen kapjon friss belső hivatkozást a legjobb linkpozícióból. A `/kerekcsere` oldal GYIK szekcióval és `FAQPage` JSON-LD sémával bővült (4 kérdés-válasz a "mikor kell téli gumit cserélni", "mennyi ideig tart egy kerékcsere" típusú, addig egyáltalán nem célzott hosszú keresésekre). Mindkettő buildelve, commitolva (`2d46ded`), élesítve; utána a főoldalra és a `/kerekcsere`-re is manuális újraindexelés kérve a Search Console-ban. Az ügyfél megerősítette, hogy a "Gumiabroncs-szerviz" kategória jóváhagyva, élesen látszik a Google Cégprofilban (a 2026-08-16-i és 2026-08-25-i audit nyitott pontjának végleges lezárása). A megosztható artifact ennek megfelelően frissítve (elvégzett tételek átvezetve).
...
- `2c196b4` (2026.08.25) - `fix(seo): a vercel.app default domain átirányítása a kanonikus domainre` – a teljes SEO/GEO audit során talált kritikus hiba javítása, lásd fent a 2026.08.25-i szakaszt.
- `2d46ded` (2026.08.25) - `feat(seo): napi rotáló kiemelt autók a főoldalon + GYIK a kerékcsere oldalon` – az audit két nyitott javaslatának megvalósítása.
